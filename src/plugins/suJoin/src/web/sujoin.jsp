...
<%  // Get parameters
    ...
    SUJoinPlugin plugin = (SUJoinPlugin) XMPPServer.getInstance().getPluginManager().getPlugin("sujoin");
...
<p>
Use the form below to enable or disable the SUJoin Service and configure the secret key.
By default the SUJoin Service plugin is <strong>disabled</strong>, which means that
...
<%  if (success) { %>
    ...
    <tbody>
        ...
        <td class="jive-icon-label">
            SUJoin service properties edited successfully.
        </td></tr>
    </tbody>
    </table>
    </div><br>
...
<fieldset>
    ...
    <ul>
        ...
        <%= ((enabled) ? "checked" : "") %>>
        <label for="rb01"><b>Enabled</b> - SUJoin service requests will be processed.</label>
        <br>
        <input type="radio" name="enabled" value="false" id="rb02"
         <%= ((!enabled) ? "checked" : "") %>>
        <label for="rb02"><b>Disabled</b> - SUJoin requests will be ignored.</label>
        <br><br>
